...
**ATTENTION:** If you have a virus scanner installed on your PC, it may sound an alarm when you start the program for the first time. Since our software is not malware, you can usually allow it to run without any problems. If you still don't trust the exe file, you can alternatively run the source code directly in Python. Please note that we are not professional software developers and have not registered our programs through code signing. However, we are working to identify the cause of the virus scanner alerts and will take this into account in future releases.
