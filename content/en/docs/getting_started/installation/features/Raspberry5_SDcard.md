...
Before the Raspberry Pi 5 can be put into operation with the COHI Player Mini, the SD card must be prepared with the operating system and the COHI Player Mini software.
...
2. Download the [Image file ](https://cohiradia.radiomuseum.org/download/software/COHI-RasPi5_V4.1_shrink.img) from the COHIRADIA software archive and save it to any directory.
...
```bash
sudo dd if=COHI-RasPi5_V4.1_shrink.img of=/dev/YOURSDDEVICE bs=4M status=progress conv=fsync
